...
def inspect_photometry(      
    ...
    X_low = np.linspace(x_min, x_max, nx)
    Y_low = np.linspace(y_min, y_max, ny)
    ...
    ax[0].text(
        ...
        f"Flux and Poisson: {flux:.2f} ± {poisson_err:.2f}",
    ...
    ax[1].text(
        0.02, 0.98,
        f"Background: mean = {np.mean(back_flux_arr):.2f}, std = {np.std(back_flux_arr):.2f}",
