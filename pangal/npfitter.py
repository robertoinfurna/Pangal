...
class NPFitter(): 
    ...
    def __init__(self, model_file='FSPS_SSPs', cosmo=None, dustemimodel='dh02', leitatt=False, uv_bump=False, emimetal=0.0017, emimodel='2018'):

        print(f"Initializing NPFitter object")
        ...
        print("Loading SSPs SEDs from ",model_file)
        ...
        with fits.open(model_file) as hdul:
            ...
            for i in range(1, len(hdul)):
                fluxes.append(hdul[i].data)
                ages.append(hdul[i].header['age'])
            
            self._get_SSP_interp = interp1d(
                ages,
                fluxes,
                kind='linear',
                axis=0,
    ...
    def get_SSP(self, age):
        """
        Return SSP flux interpolated in age and onto self.model_wl.
        """
        # 1. Get flux at requested age (on original SSP grid)
        flux_age = self._get_SSP_interp(age)  # shape: (n_wl_orig,)

        # 2. Interpolate flux onto extended wavelength grid
        # Make a linear interpolator in wavelength
        f_interp = interp1d(
            self.model_wl[:len(flux_age)],  # only original wl part
            flux_age,
            kind='linear',
            bounds_error=False,
            fill_value="extrapolate"
        )
        flux_on_model_wl = f_interp(self.model_wl)

        return flux_on_model_wl  # shape: (len(self.model_wl),)
    ...
    def synthetic_spectrum(
        ...
        # --- Stellar spectra ---
        young_stellar_spec = np.zeros(len(self.model_wl))
        old_stellar_spec   = np.zeros(len(self.model_wl))
        dt = np.diff(np.insert(time_bins, 0, 0))

        # usefull if multi_component
        stellar_mass = []
        ssp_components = []

        for j,age in enumerate(time_bins):

            mass_formed = sfh_weights[j] * dt[j]  # total mass formed in this bin

            SSP_sed = mass_formed  * self.get_SSP(age)

            if age <= 10:
                young_stellar_spec += SSP_sed
            else:
                old_stellar_spec += SSP_sed

            if multi_component: 
                stellar_mass.append(mass_formed)
                ssp_components.append(SSP_sed)



        total_mass_formed = np.sum(sfh_weights * dt)
        ...
        parsec = 3.0857e18
        L_cm = luminosity_distance * parsec * 1e6
        fscale = 1/total_mass_formed * 10**m_star / (4*np.pi * L_cm**2)
        total_spec *= fscale
        ...
        if not likelihood_call:
            ...
            if multi_component:
                ...
                # --- Save SSP components ---
                spec.ssp_components = []
                for k, mass in enumerate(stellar_mass):
                    component_header = header.copy()
                    component_header["COMPONENT"] = f"SSP of age {time_bins[k]:.2f} Myr"
                    component_header["AGE"] = f"{time_bins[k]:.2f}"
                    component_header["STELLAR MASS FORMED"] = mass

                    # scale flux to observed frame
                    fl = ssp_components[k] / (1 + redshift) * fscale
                    ...
                    spec.ssp_components.append(ssp_spec)
            return spec
